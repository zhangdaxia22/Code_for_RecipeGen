...
def generate_t2v_videos_for_dishes(dish_base_dir, temp_dir='./samples'):
    """
    为每个菜品文件夹中的 steps.txt 生成文生视频，保存到 video_256px 目录，完成后重命名为 videos。
    如果目标视频已存在，则跳过该步骤的生成。
    :param dish_base_dir: 菜品文件夹根目录（如 '/root/autodl-tmp/Open-Sora/sampled-yooucook2-cleaned'）
    :param temp_dir: inference.py 生成视频的临时文件夹（如 './samples'）
    """
    # 初始化 pynvml
    ...
    for dish_idx, dish_id in enumerate(tqdm(dish_ids, desc="处理菜品文件夹", position=0), 1):
        ...
        for step in tqdm(steps, desc=f"处理 {dish_id} 的步骤", leave=False, position=1):
            ...
            target_path_256px = os.path.join(video_dir, target_name)
            
            # 检查 videos 和 video_256px 文件夹中是否已存在目标视频
            if os.path.exists(target_path) or os.path.exists(target_path_256px):
                print(f"目标视频已存在（videos 或 video_256px），跳过: {target_name} (菜品: {dish_id})")
                continue
...
def main():
    ...
    temp_dir = './samples'  # 临时文件夹，与指令中的 --save-dir 一致
    """
    sample_file = '/root/autodl-tmp/douguo/sampled_dishes.txt'
    dataset_base_dir = '/root/autodl-tmp/douguo/test'
    output_base_dir = '/root/autodl-tmp/douguo/t2v-opensora-douguo'
    temp_dir = '/root/autodl-tmp/douguo/results_tmp'
    """
    # 执行生成
    generate_t2v_videos_for_dishes(dish_base_dir, temp_dir)
    print("所有视频生成和文件复制完成！")
